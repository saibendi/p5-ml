...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool empty_impl(const Node *node) {
      return node == nullptr;   // node left ptr and right ptr don't exist
  }
  ...
  static Node *copy_nodes_impl(Node *node) {
      ...
      Node* new_node = new Node(node->datum, nullptr, nullptr);
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
      if (node == nullptr) {   // BASE CASE: if tree is empty OR if end of tree
          return nullptr;
      }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
      assert(find_impl(node, item, less) == nullptr); // requiring that node isn't in tree
      if (node == nullptr) {
